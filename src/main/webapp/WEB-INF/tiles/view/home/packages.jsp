<div class="container">
    <div class="page-header clearfix">
        <h2 class="page-name pull-left">Packages</h2>
        <nav class="pull-right">
            <ul class="pagination">
                <li><a href="#">&laquo;</a></li>
                <li><a href="#">1</a></li>
                <li><a href="#">2</a></li>
                <li><a href="#">3</a></li>
                <li><a href="#">4</a></li>
                <li><a href="#">5</a></li>
                <li><a href="#">&raquo;</a></li>
            </ul>
        </nav>
    </div>
    <div class="package-grid">
        <div class="package">
            <div class="package-img">
                <img src="/static/img/dress_1.jpg" class="img-200x300"/>
            </div>
            <div class="package-name">Pamel Roland</div>
            <div class="package-desc">
                Wikipedia is written collaboratively by largely anonymous Internet volunteers who write without pay.
                Anyone with Internet access can write and make changes to
                <div class="package-desc-shadow"></div>
            </div>
            <div class="btn btn-sm btn-default">Details</div>
        </div>
        <div class="package">
            <div class="package-img">
                <img src="/static/img/dress_2.jpg" class="img-200x300"/>
            </div>
            <div class="package-name">Pamel Roland</div>
            <div class="package-desc">
                Wikipedia is written collaboratively by largely anonymous Internet volunteers who write without pay.
                Anyone with Internet access can write and make changes to
                <div class="package-desc-shadow"></div>
            </div>
            <div class="btn btn-sm btn-default">Details</div>
        </div>
        <div class="package">
            <div class="package-img">
                <img src="/static/img/dress_3.jpg" class="img-200x300"/>
            </div>
            <div class="package-name">Pamel Roland</div>
            <div class="package-desc">
                Wikipedia is written collaboratively by largely anonymous Internet volunteers who write without pay.
                Anyone with Internet access can write and make changes to
                <div class="package-desc-shadow"></div>
            </div>
            <div class="btn btn-sm btn-default">Details</div>
        </div>
        <div class="package">
            <div class="package-img">
                <img src="/static/img/dress_4.jpg" class="img-200x300"/>
            </div>
            <div class="package-name">Diane von Furstenberg</div>
            <div class="package-desc">
                Wikipedia is written collaboratively by largely anonymous Internet volunteers who write without pay.
                Anyone with Internet access can write and make changes to
                <div class="package-desc-shadow"></div>
            </div>
            <div class="btn btn-sm btn-default">Details</div>
        </div>
        <div class="package">
            <div class="package-img">
                <img src="/static/img/dress_5.jpg" class="img-200x300"/>
            </div>
            <div class="package-name">Pamel Roland</div>
            <div class="package-desc">
                Wikipedia is written collaboratively by largely anonymous Internet volunteers who write without pay.
                Anyone with Internet access can write and make changes to
                <div class="package-desc-shadow"></div>
            </div>
            <div class="btn btn-sm btn-default">Details</div>
        </div>
        <div class="package">
            <div class="package-img">
                <img src="/static/img/dress_6.jpg" class="img-200x300"/>
            </div>
            <div class="package-name">Pamel Roland</div>
            <div class="package-desc">
                Wikipedia is written collaboratively by largely anonymous Internet volunteers who write without pay.
                Anyone with Internet access can write and make changes to
                <div class="package-desc-shadow"></div>
            </div>
            <div class="btn btn-sm btn-default">Details</div>
        </div>
    </div>
</div>